...
extension UINavigationBar
{
    ...
    fileprivate func yl_setBackgroundImage(image:UIImage)
    {
    ...
    fileprivate func yl_setBackgroundColor(color:UIColor)
    {
    ...
    fileprivate func yl_setBackgroundAlpha(alpha:CGFloat)
    {
    ...
    func yl_setBarButtonItemsAlpha(alpha:CGFloat, hasSystemBackIndicator:Bool)
    {
    ...
    /// 设置导航栏在垂直方向上平移多少距离
    func yl_setTranslationY(translationY:CGFloat)
    {
        transform = CGAffineTransform.init(translationX: 0, y: translationY)
    }
    
    func yl_getTranslationY() -> CGFloat
    {
        return transform.ty
    }
    ...
    open override class func initialize()
    {
        ...
        DispatchQueue.once(token: onceToken)
        {
            ...
            for selector in needSwizzleSelectorArr {
                let str = ("yl_" + selector.description)
    ...
    // MARK: swizzling pop
    //==========================================================================
    func yl_setTitleTextAttributes(_ newTitleTextAttributes:[String : Any]?)
    {
        ...
        guard let originTitleTextAttributes = titleTextAttributes else {
            yl_setTitleTextAttributes(attributes)
            return
        }
...
extension UINavigationController
{
    ...
    {
        navigationBar.yl_setBackgroundImage(image: backgroundImage)
    }
    
    fileprivate func setNeedsNavigationBarUpdate(barTintColor: UIColor)
    {
        navigationBar.yl_setBackgroundColor(color: barTintColor)
    }

    fileprivate func setNeedsNavigationBarUpdate(barBackgroundAlpha: CGFloat)
    {
        navigationBar.yl_setBackgroundAlpha(alpha: barBackgroundAlpha)
    }
    ...
    open override class func initialize()
    {
        ...
        DispatchQueue.once(token: onceToken)
        {
            ...
            for selector in needSwizzleSelectorArr {
                // _updateInteractiveTransition:  =>  yl_updateInteractiveTransition:
                let str = ("yl_" + selector.description).replacingOccurrences(of: "__", with: "_")
                let originalMethod = class_getInstanceMethod(self, selector)
                let swizzledMethod = class_getInstanceMethod(self, Selector(str))
                method_exchangeImplementations(originalMethod, swizzledMethod)
            }
    ...
    func yl_popToViewController(_ viewController: UIViewController, animated: Bool) -> [UIViewController]?
    {
        ...
        CATransaction.begin()
        let vcs = yl_popToViewController(viewController, animated: animated)
        CATransaction.commit()
        return vcs
    }
    
    // swizzling system method: popToRootViewControllerAnimated
    func yl_popToRootViewControllerAnimated(_ animated: Bool) -> [UIViewController]?
    {
        var displayLink:CADisplayLink? = CADisplayLink(target: self, selector: #selector(popNeedDisplay))
        displayLink?.add(to: RunLoop.main, forMode: .commonModes)
        CATransaction.setCompletionBlock {
        ...
        CATransaction.setAnimationDuration(popProperties.popDuration)
        CATransaction.begin()
        let vcs = yl_popToRootViewControllerAnimated(animated)
        CATransaction.commit()
        return vcs;
    }
    ...
    func yl_pushViewController(_ viewController: UIViewController, animated: Bool)
    {
        ...
        CATransaction.begin()
        yl_pushViewController(viewController, animated: animated)
        CATransaction.commit()
    }
...
extension UINavigationController: UINavigationBarDelegate
{
    ...
    func yl_updateInteractiveTransition(_ percentComplete: CGFloat)
    {
        guard let topViewController = topViewController,
            let coordinator       = topViewController.transitionCoordinator else {
                yl_updateInteractiveTransition(percentComplete)
                return
        }
...
extension UIViewController
{
    ...
    var navBarBackgroundImage: UIImage? {
        ...
        set {
            if customNavBar.isKind(of: UINavigationBar.self) {
                ...
                navBar.yl_setBackgroundImage(image: newValue!)
            }
            else
            {
    ...
    var navBarBarTintColor: UIColor {
        ...
        set {
            ...
            if customNavBar.isKind(of: UINavigationBar.self) {
                let navBar = customNavBar as! UINavigationBar
                navBar.yl_setBackgroundColor(color: newValue)
            }
            else
            {
    ...
    var navBarBackgroundAlpha:CGFloat {
        ...
        set {
            ...
            if customNavBar.isKind(of: UINavigationBar.self) {
                let navBar = customNavBar as! UINavigationBar
                navBar.yl_setBackgroundAlpha(alpha: newValue)
            }
            else
            {
                if pushToCurrentVCFinished == true && pushToNextVCFinished == false {
    ...
    open override class func initialize()
    {
        ...
        DispatchQueue.once(token: onceToken)
        {
            ...
            {
                let newSelectorStr = "yl_" + selector.description
                let originalMethod = class_getInstanceMethod(self, selector)
    ...
    func yl_viewWillAppear(_ animated: Bool)
    {
        ...
        yl_viewWillAppear(animated)
    }
    
    func yl_viewWillDisappear(_ animated: Bool)
    {
        if canUpdateNavigationBar() == true {
            pushToNextVCFinished = true
        }
        yl_viewWillDisappear(animated)
    }
    
    func yl_viewDidAppear(_ animated: Bool)
    {
        if canUpdateNavigationBar() == true
        {
